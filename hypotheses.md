...
|---|-----------|------------------------|--------|
| 1 | **Tournées acide/base retenues** | Déterminent les capacités journalières, les distances et les besoins de flotte | ✅ Scénario hybride S3/S4 retenu provisoirement (`test_annuel.md`) |
| 2 | **Taux d'amortissement α** | Paramètre fixe pour calculer la recette de revente `C/(1+α)^n` (ce qu'on optimise c'est *quand* vendre, pas α) | ✅ α = 0,20 par hypothèse sourcee + sensibilité |
...
| 8 | **Durée de travail journalière** | Détermine la faisabilité des doubles tournées et le coût chauffeur | ✅ 9 h/jour/camion |
| 9 | **Coût chauffeur** | Les salaires doivent couvrir conduite, chargement, livraison et déchargement | ✅ 35 €/h HTVA |
...
`test_annuel.md`. Ce fichier compare quatre organisations combinant acides et
bases, dont trois lectures journalieres et une lecture annuelle avec changement
d'affectation.

### 2.1 Convention journalière

- **Jours ouvrables** : 250 jours/an.
- **Temps de travail principal** : 9 h/jour/camion.
...
- **Rechargement à Liège avant une deuxième tournée** : 0,5 h.
...
Le scénario journalier recommandé dans `test_annuel.md` est le **scénario 3
hybride**. La variante annuelle S4 devient préférable si on accepte de planifier
la base restante sur l'année :
...
### 2.5 Points de vigilance à signaler

- **Convention 9 h/jour** : le modèle final retient 9 h/jour/camion. Cette
  convention rend faisables les doubles tournées acide étudiées dans
  `tests_acide_type1_9h_rechargement.md`.
- **Rechargement acide à Liège** : lorsqu'un camion revient à Liège et repart
  pour une deuxième tournée dans la même journée, on ajoute 0,5 h de
  rechargement. Le premier chargement de la journée est supposé intégré dans
...
- **Heures de travail annuelles maximales** : `H^max = 2250 h/an`
  → Justification : 250 jours ouvrables × 9 h/jour = 2250 h/an.
- **Rechargement entre deux tournées** : `t^reload = 0,5 h` si un camion revient
  à Liège et repart pour une deuxième tournée dans la même journée.
...
- **Coût chauffeur** : 35 €/h HTVA par camion en service.
  - Ce coût s'applique à tout le temps de travail : conduite, livraison,
    chargement de base à Anvers, déchargement de base à Liège et rechargement à
    Liège avant une deuxième tournée.
  - Pour une tournée `r`, le coût chauffeur est donc :
    `C_chauffeur,r = 35 × T_r`, où `T_r` est le temps total de la tournée.
  - Pour le PL annuel, on retient l'approximation simple :
    `C_chauffeur,t = 35 × 9 × 250 × (N_{1,t} + N_{2,t})`.
- **Coût kilométrique hors péage** : 0,60 €/km HTVA.
  - Il couvre principalement carburant, pneus et consommables liés au roulage.
  - Les péages et la redevance kilométrique belge sont ignorés dans le modèle.
- **Revente** : `C / (1+α)^n`
  - **Taux d'amortissement α** : non donné par l'énoncé (indiqué seulement "entre 0 et 1").
    - **Choix retenu : α = 0,20.**
    - Justification : les sources proches du contexte indiquent une durée de détention d'environ 7 ans pour les tracteurs dans le transport routier belge (CNR) et une référence comptable belge de 5 ans pour les véhicules professionnels. Dans la formule de l'énoncé, `α = 0,20` donne une valeur de revente d'environ 40 % après 5 ans et 28 % après 7 ans.
    - Une analyse de sensibilité sur α ∈ {0,15 ; 0,20 ; 0,25} sera menée.
...
- **Un camion en service mobilise un chauffeur** : on ne modélise pas le planning
  individuel des chauffeurs, mais on ajoute un coût horaire chauffeur aux heures
  travaillées par camion.
- **Pas d'immobilisation de changement de compartiment** : les affectations sont
  fixes à l'année, donc les 3 jours de nettoyage ne sont pas déclenchés dans le
  modèle principal.
...
- [x] Valeur de `α` → 0,20 (sources CNR + amortissement belge, §6)
...
- [x] Durée journalière → 9 h/jour/camion (§2)
- [x] Coût chauffeur → 35 €/h HTVA, appliqué aux temps de conduite, chargement, livraison et déchargement (§6)
- [x] Immobilisation de changement de compartiment → ignorée car pas de changement en cours d'année (§3, §9)
